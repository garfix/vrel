...
def get_read_grammar1():
    return [
        ...
        # X is a Y
        {
            ...
            "sem": lambda noun, is1, a, np: [("let", T1, "true")] + noun + [("intent_tell", np)],
        },
        # X is Y (X is another name for Y)
        {
            "syn": "s() -> proper_noun(E1) 'is' proper_noun(E1)",
            "sem": lambda proper_noun1, proper_noun2: proper_noun1 + proper_noun2 + [("intent_tell", [])],
        },
        # no X is a Y
        {
            "syn": "s() -> 'no' noun(E1, T1) is() a() np(E1, T2)",
            "sem": lambda noun, is1, a, np: [("let", T1, "true"), ("let", T2, "false"), ("intent_learn", np[0], noun)],
        },
        # a X is not a Y
        {
        ...
        # combustable things burn
        {
            ...
            "sem": lambda np, verb: [("let", T1, "true"), ("let", T2, "true"), ("intent_learn", verb[0], np)],
        },
        # dark-gray things are not white
        {
            "syn": "s() -> np(E1, T1) are() 'not' adj(E1, T2)",
            "sem": lambda np, are, adj2: [("let", T1, "true"), ("let", T2, "false"), ("intent_learn", adj2[0], np)],
        },
        # gasoline is combustable
        {
            "syn": "s() -> noun(E1, T1) 'is' adj(E1, T1)",
            "sem": lambda noun, adj: [("let", T1, "true"), ("let", T2, "true"), ("intent_learn", adj[0], noun)],
        },
        # X's are not Y's
        {
            "syn": "s() -> noun(E1, T1) are() 'not' np(E1, T2)",
            "sem": lambda noun, are, np: [
                ("scoped", [("let", T1, "false"), ("let", T2, "true"), ("intent_learn", noun[0], np)]),
                ("scoped", [("let", T1, "true"), ("let", T2, "false"), ("intent_learn", np[0], noun)]),
            ],
        },
        # X's are Y's
        {
            "syn": "s() -> noun(E1, T1) are() np(E1, T2)",
            "sem": lambda noun, are, np: [("let", T1, "true"), ("let", T2, "true"), ("intent_learn", np[0], noun)],
        },
        # any thing that burns rapidly burns
        {
            "syn": "s() -> 'any' 'thing' 'that' verb(E1) verb(E1)",
            "sem": lambda verb1, verb2: [("let", T1, "true"), ("let", T2, "true"), ("intent_learn", verb2[0], verb1)],
        },
        # np
        ...
        },
        {
            ...
            "sem": lambda token, main_noun: [("resolve_name", token + " " + main_noun, E1)],
        },

...
def get_read_grammar2():
    return [
        ...
        # vp
        {
            "syn": "vp(E1, T3) -> np(E1, T2) is() a() np(E1, T1)",
            "sem": lambda np1, is1, a, np2: np1 + np2 + [("and_3v", T1, T2, T3)],
        },
        {
            "syn": "vp(E1, T3) -> np(E1, T2) is() 'not' a() np(E1, T1)",
            "sem": lambda np1, is1, a, np2: np1 + np2 + [("not_3v", T1, T3)],
        },
        {
            "syn": "vp(E1, T3) -> vp(E1, T1) rel_clause(E1, T2)",
            "sem": lambda vp, rel_clause: vp + rel_clause + [("and_3v", T1, T2, T3)],
        },
        {
            "syn": "vp(E1, T3) -> np(E1, T1) is() np(E1, T2)",
            "sem": lambda np1, is1, np2: np1 + np2 + [("and_3v", T1, T2, T3)],
        },
        {
            "syn": "vp(E1, T3) -> 'some' np(E1, T1) are() np(E1, T2)",
            "sem": lambda np1, are, np2: np1 + np2 + [("and_3v", T1, T2, T3)],
        },
        # no oxide is white
        # if E1 is an oxide and E1 is white, then 'false', else 'unknown'
        {
            "syn": "vp(E1, T3) -> 'no' np(E1, T1) is() np(E1, T2)",
            "sem": lambda np1, is1, np2: [("let", T1, "true")]
            + [("let", T2, "true")]
            + np1
            + np2
            + [("not_3v", T2, T3)],
        ...
        {
            "syn": "vp(E1, T3) -> np(E1, T1) are() 'not' np(E1, T2)",
            "sem": lambda np1, are, np2: [("let", T1, "true")]
            + [("let", T2, "true")]
            + np1
            + np2
            + [("not_3v", T2, T3)],
        ...
        {
            "syn": "vp(E1, T3) -> 'every' np(E1, T1) is() a() np(E1, T2)",
            "sem": lambda np1, is1, a, np2: [("let", T1, "true")]
            + [("let", T2, "true")]
            + np1
            + np2
            + [("and_3v", T1, T2, T3)],
        ...
        # ferrous sulfide is not a compound that is not dark-gray
        {
            ...
            "sem": lambda np1, is1, a, np2, is2, np3: np1
            + np2
            + np3
            + [("not_3v", T3, T4), ("and_3v", T2, T4, T5), ("not_3v", T5, T6)],
        },
        ...
        # anything that is not a compound is not ferrous sulfide
        {
            ...
            "sem": lambda np1, is2, np2: np1
            + np2
            + [("and_3v", T1, T2, T3), ("not_3v", T3, T4)],
        },
        # no dark gray thing is a sulfide
        {
            "syn": "vp(E1, T4) -> 'no' adj(E1, T1) 'thing' is() a() np(E1, T2)",
            "sem": lambda adj, is1, a, np: adj
            + np
            + [("and_3v", T1, T2, T3), ("not_3v", T3, T4)],
        },
        # gasoline is a fuel that burns
        {
            ...
            "sem": lambda np1, is1, a, np2, verb: np1
            + np2
            + verb
            + [("and", T1, T2, T4), ("and_3v", T3, T4, T5)],
        },
        # article
        {"syn": "a() -> 'a'", "sem": lambda: []},
        {"syn": "a() -> 'an'", "sem": lambda: []},
        ...
        {
            "syn": "verb(E1, T1) -> 'burns' 'rapidly'",
            "sem": lambda: [("burns_rapidly", E1, T1)],
        },
        # copula
        ...
        {
            "syn": "np(E1, T1) -> adj(E1, T1) np(E1, T1)",
            "sem": lambda adj, noun: adj + noun,
        },
        {
            "syn": "np(E1, T1) -> proper_noun(E1)",
            "sem": lambda proper_noun: proper_noun + [("let", T1, "true")],
        },
        {
            "syn": "np(E1, T2) -> 'anything' 'that' is() 'not' a() np(E1, T1)",
            "sem": lambda is1, a, np: np + [("not_3v", T1, T2)],
        },
        # adjective

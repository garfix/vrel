...
from vrel.entity.GrammarRule import GrammarRule
from vrel.entity.GrammarRules import GrammarRules
from vrel.entity.RuleConstituent import RuleConstituent
from vrel.processor.parser.earley.EarleyParser import EarleyParser
...
class TestEarleyParser(unittest.TestCase):
    ...
    def test_ambiguity(self):
        ...
        self.assertEqual(
            result.products[0].inline_str(),
            "s(np(noun(john 'john')) verb(loves 'loves') np(noun(proper_noun(mary 'mary'))))",
        )
        self.assertEqual(
            result.products[1].inline_str(),
            "s(np(noun(proper_noun(john 'john'))) verb(loves 'loves') np(noun(proper_noun(mary 'mary'))))",
        )
        self.assertEqual(
            result.products[2].inline_str(),
            "s(np(noun(john 'john')) vp(verb(loves 'loves') np(noun(proper_noun(mary 'mary')))))",
        )
        self.assertEqual(
            result.products[3].inline_str(),

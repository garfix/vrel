...
class TestMultilingual(unittest.TestCase):
    ...
    def test_two_languages(self):
        ...
        parser = BasicParser(
            grammar_parser.parse_read_grammar(
                [
                    # English
        ...
        system = BasicSystem(parser=parser)
        ...
        self.assertEqual(
            response.products[0].parse_trees[0].inline_str(),
            "s(np(noun(proper_noun(jan 'jan'))) vp(verb(houdt 'houdt' van 'van') np(noun(proper_noun(marie 'marie')))))",
        )

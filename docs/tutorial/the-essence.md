...
## A basic system, extendible by modules
...
One of the ways to achieve this is to leave quantification out of the composition process, and offer a function that transforms the structure into a quantified form. The composition, which is done manually is therefore easier, while the transformation is done by a built-in function.
...
The advantages may not seem to weigh up against the disadvantages. Many natural language sentences can't be solved by a single database query, but by an interaction between data access and data processing. That's where this approach shines.
